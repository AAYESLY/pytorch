...
class ReductionInvariantIndexingTests(InductorTestCase):
    ...
    def test_reduction_invariant_masked_index_load(self, persistent_reductions):
        ...
        kernel = kernels[0]
        narrow_load = r"tl\.load\([^\n]*\[XBLOCK, 1\]"
        FileCheck().check_regex(narrow_load).check_regex(narrow_load).run(kernel)
    ...
    def test_reduction_invariant_masked_index_expr(self, persistent_reductions):
        ...
        self.assertEqual(1, len(kernels))
        kernel = kernels[0]
        FileCheck().check_regex(r"tl\.broadcast_to\(x\d+, \[XBLOCK, 1\]\)").run(kernel)
        if persistent_reductions:
    ...
    def test_reduction_invariant_load_direct_reduction(self):
        ...
        self.assertEqual(1, len(kernels))
        narrow_load = r"tl\.load\([^\n]*\[XBLOCK, 1\]"
        FileCheck().check_regex(narrow_load).check_regex(narrow_load).check(
            "tl.broadcast_to"
        ).check("[XBLOCK, R0_BLOCK]").check("tl.sum").run(kernels[0])
    ...
    def test_reduction_invariant_indirect_load(self, persistent_reductions):
        ...
        kernel = kernels[0]
        narrow_load = r"tl\.load\([^\n]*\[XBLOCK, 1\]"
        FileCheck().check_regex(narrow_load).check_regex(narrow_load).check_regex(
            narrow_load
        ).check_regex(narrow_load).run(kernel)
        if persistent_reductions:
            FileCheck().check("@triton_heuristics.persistent_reduction").run(kernel)
        else:
            FileCheck().check("for r0_offset in tl.range").run(kernel)
    ...
    def test_combo_reduction_invariant_pointwise_load(self):
        ...
        narrow_load = r"tl\.load\([^\n]*\[YBLOCK, 1, 1\]"
        FileCheck().check_regex(narrow_load).check_regex(narrow_load).run(code[0])
    ...
    def test_reduction_dependent_mask_keeps_masked_index_load_dense(self):
        ...
        self.assertEqual(fn(*inputs), actual)
        self.assertEqual(1, len(kernels))
        FileCheck().check_regex(r"tl\.load\(in_ptr0[^\n]*R0_BLOCK").run(kernels[0])
