...
class TestDedupeGraphOutputs(TestCase):
    ...
    def test_effectful_graphs_fail_closed(self):
        ...
        for target in (torch.ops.aten.rand_like.default, torch.ops.aten.empty_like.default):
            with self.subTest(target=target):
                graph = fx.Graph()
                x = graph.placeholder("x")
                graph.output(
                    tuple(graph.call_function(target, (x,)) for _ in range(8))
                )
                gm = fx.GraphModule({}, graph)
                _propagate(gm, torch.randn(8))
                dedupe_graph_outputs_pass(graph)
    ...
    def test_collectives_fail_closed(self):
        ...
        graph.output(
            tuple(
                graph.call_function(target, (x, "sum", "0")) for _ in range(8)
            )
        )
        gm = fx.GraphModule({}, graph)
        _propagate(gm, torch.randn(8))
    ...
    def test_uninitialized_and_special_ops_are_not_shareable(self):
        ...
        nodes = [
            ...
            graph.call_function(torch.ops.aten.empty_quantized.default, ((8,), qtensor)),
            graph.call_function(
    ...
    def test_prims_and_view_branches_are_shared(self):
        ...
        dedupe_graph_outputs_pass(graph)
        self.assertEqual(
            _count(graph, torch.ops.prims.convert_element_type.default), 1
        )
        self.assertEqual(_count(graph, torch.ops.aten.clone.default), 7)
