...
from torch._inductor.sizevars import is_range_bound
...
class AOTInductorTestsTemplate:
    ...
    def test_scalar_range_asserts_disabled_drops_inferred_bound(self):
        ...
        with config.patch({"scalar_range_asserts": False}):
            _, code = run_and_get_cpp_code(
                AOTIRunnerUtil.legacy_compile, model, example_inputs
            )
    ...
    def test_shape_contracts_are_not_range_bounds(self, expr_str) -> None:
        # Tie quantities together rather than bounding one against a constant.
        # A lowering cannot produce most of them, hence testing the predicate.
        self.assertFalse(is_range_bound(sympy.sympify(expr_str)))

    @common_utils.parametrize(
        "expr_str", ["u0 >= 4", "u0 < 10", "4 < u0", "u0 <= 10", "s0 >= 100"]
    )
    def test_bound_against_constant_is_a_range_bound(self, expr_str) -> None:
        # Without this the suite passes on the feature deleted.
        self.assertTrue(is_range_bound(sympy.sympify(expr_str)))
