...
struct C10_API PyInterpreterVTable {
  ...
  // try a registered Python decomposition for op. With static shapes only
  // torch._decomp decompositions with non-sparse inputs run, matching Python
  // FakeTensorMode's `has_symbolic_sizes or torch_decomp_decompositions(func)`.
