...
def is_output_computation_sharing_supported(gm: fx.GraphModule) -> bool:
    ...
    devices = {
        leaf.device
        for arg in torch.utils._pytree.arg_tree_leaves(*output.args, **output.kwargs)
...
def _is_shareable_node(node: fx.Node) -> bool:
    if (
        not _is_functional_op(node)
        or node.target.overloadpacket in _REJECTED_PACKETS
        or not _REJECTED_TAGS.isdisjoint(node.target.tags)
    ):
        return False
    return not any(
...
def _storage_keys(value) -> set[_StorageKey]:
    return {
        key
...
def _has_distinct_dense_storage(
    node: fx.Node,
    input_storage: set[_StorageKey],
    output_storage_users: dict[_StorageKey, set[fx.Node]],
) -> bool:
...
def dedupe_graph_outputs_pass(graph: fx.Graph) -> None:
    ...
    input_storage: set[_StorageKey] = set()
    for node in graph.nodes:
        if node.op in ("placeholder", "get_attr"):
            input_storage.update(_storage_keys(node.meta.get("val")))

    output_storage_users: dict[_StorageKey, set[fx.Node]] = defaultdict(set)
    for node in output_nodes:
    ...
    for group in candidates:
        eligible = [
            node
            for node in group
            if _has_distinct_dense_storage(
                node, input_storage, output_storage_users
            )
        ]
